import { AttributeData, Context, ElementsBundle, Index, Program, UniformData } from "./rendering/engine/engine.core";
import { setup3dScene } from "./rendering/engine/webgl";
...
export class WebGlPolygonRenderer extends LayerRenderer<VectorLayer> {
    ...
    constructor(layer: VectorLayer, private colorFunc: (f: Feature<Polygon>) => number[], data?: PolygonRendererData) {
        super(layer);
    ...
    updateData(features: Feature<Polygon>[]) {
        const data = parseFeaturesToRendererData(features, this.colorFunc);
        this.polyShader.bind(this.context);
        this.polyShader.updateAttributeData(this.context, 'a_coord', data.coords.data);
        this.polyShader.updateAttributeData(this.context, 'a_color', data.colors.data);
        this.polyShader.updateIndex(this.context, data.polyIndex.data);
        this.polyShader.upload(this.context);
        this.lineShader.bind(this.context);
        this.lineShader.updateAttributeData(this.context, 'a_coord', data.coords.data);
        this.lineShader.updateAttributeData(this.context, 'a_color', data.colors.data);
        this.lineShader.updateIndex(this.context, data.lineIndex.data);
        this.lineShader.upload(this.context);
    }
...
export class WebGlPolygonLayer extends VectorLayer {
    ...
    private renderer: WebGlPolygonRenderer;

    constructor(opt_options: WebGlPolygonLayerOptions) {
        super(opt_options);
        this.renderer = new WebGlPolygonRenderer(this, opt_options.colorFunc, opt_options.webGlData);
    }
    ...
    setSource(source: VectorSource<Polygon>) {
        super.setSource(source);
        const features = source.getFeatures();
        if(this.renderer) {
            this.renderer.updateData(features);
        }
    }
}
